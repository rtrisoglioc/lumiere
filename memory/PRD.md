...
### Shot List discoverability + full Spanish translation (2026-09-09) — DONE, self-tested
User: on the SHOT LIST couldn't find edit/delete/update and the Spanish translation wasn't visible.
- `Experience.jsx`: shot-card action buttons (edit / update-with-AI / delete) are now ALWAYS visible (removed `opacity-0 group-hover`) → discoverable. Time window enum now localized via `winLabel()` map (golden_hour_am/pm, morning, midday, afternoon, blue_hour, night, any → EN/ES) instead of raw "MIDDAY"/"GOLDEN_HOUR_PM"; when a computed clock time exists it shows `localizedWindow · time`.
- Translation extended to cover the shot header: `v2.py::translate_story` payload + apply loop now include `shot_type` and `movement` (was action/composition_note/narrative_purpose only); `v2agents.TRANSLATE_SYS` updated. Verified via API: shot_type/movement/action all render in Spanish ("PRIMER PLANO · SEGUIMIENTO HACIA ADELANTE", "Sostén el teléfono…"); 10 always-visible edit/regen/delete buttons confirmed by screenshot with lang=es.

### Black videos + can't build fix (2026-09-09) — DONE, self-tested
User: demo/footage videos rendered BLACK, and "Your Film" (build) seemed to not work.
- Root cause: mp4s had the moov atom at the END (not faststart) AND GET /api/files/{path} returned plain 200 with NO HTTP Range support -> Chrome couldn't decode -> black. (Same bug class fixed earlier for AI/Veo via _ranged_video_response, but /api/files used by all v2 footage+cuts never had Range.)
- Fixes: server.py::get_file now returns _ranged_video_response for video content-types (206 + Accept-Ranges), added request:Request. ffmpeg_worker.faststart_bytes() stream-copy remux to +faststart, applied at demo_load + upload ingest. One-time migration remuxed 15/20 stored assets in place. Frontend videos got preload=metadata playsInline.
- Verified: demo clip served faststart (moov before mdat) + valid h264/aac; /api/files 200 w/ Accept-Ranges and 206 on Range; build (cinematic) produced playable 6.8MB faststart cut served with Range. (Headless Chromium lacks H.264, so validated by bytes/headers not screenshot.)

### Film Editor on v2 cuts (2026-09-09) — DONE, self-tested (curl + UI)
User: "Your Film" had no editing (orientation, effects, transitions, speed, text too big / not repositionable). Confirmed scope: global editor (1f) now, per-clip timeline deferred (2a), non-destructive new version (3a).
- Backend `POST /api/v2/cuts/{cut_id}/edit` (CutEditIn): reuses `video_editor.transform_video` for aspect/orientation (16:9/9:16/1:1), color look (none/cinematic/warm/cool/bw/vivid), global speed (0.25–4×), whole-film end fade, and TITLE TEXT overlay (content + position top/center/bottom + size small/medium/large, auto-fit width). Optional SCENE transition (cut/fade/fadeblack/dissolve) re-renders from the stored `edl_json.clips` via `_render_edl(..., transition, tdur)` when ≥2 clips. Output = new child cut_version (parent_cut_id, kind="edit", edit_opts saved), faststart.
- `ffmpeg_worker._drawtext`: baked title-card fontsize 52→34, y offset reduced (fixes "text too big").
- Frontend `Experience.jsx`: "Editar" button per cut opens a bilingual modal (pills for all controls + title text toggle/input/position/size). Apply → new version appears in the cuts list. testids: edit-cut-<id>, cut-editor-modal, edit-aspect/look/speed/scenetr/endfade/textpos/textsz-*, edit-text-toggle/content, edit-apply.
- Verified: curl edit (9:16 + cinematic + 1.5× + fadeblack + text) -> 720x1280 faststart child cut; UI Apply (9:16 + vivid) created a new kind=edit version end-to-end.
- DEFERRED (agreed 2a): per-clip timeline (split, per-clip trim/speed/transition, reorder).
